...
[`oniguruma`](../oniguruma) FFI package. On IO platforms it tokenizes with the
real Oniguruma C library instead of shiki_flutter's pure-Dart engine.

It exists because `shiki_flutter`'s engine is pluggable: `ShikiHighlighterEngine`
is the seam, `ShikiHighlighterDartEngine` is the default, and this package
provides `ShikiHighlighterFFIEngine`.
...
Or per highlighter: `createHighlighter(engine: ShikiHighlighterFFIEngine())`.

Flutter apps must enable native assets: `flutter config --enable-native-assets`.

## Status: experimental — not yet at parity

The pure-Dart engine (`ShikiHighlighterDartEngine`, the default) is
golden-tested byte-for-byte against real Shiki and is the trustworthy default on
every platform. This FFI backend is **not yet a drop-in equal** — prefer the
default unless you have measured a need for the native speedup on IO and have
verified the grammars you use aren't affected by the limitation below.
...
The `oniguruma` package drives Oniguruma in **UTF-16LE** so match offsets line
