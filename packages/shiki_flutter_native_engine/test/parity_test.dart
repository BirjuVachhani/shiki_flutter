...
// `oniguruma_native` drives Oniguruma in UTF-8 (mapping match offsets back to
// UTF-16 code units for Dart String indices), so the 2-digit `\xHH` byte escapes
// TextMate grammars use for ASCII ranges (e.g. CSS's `[^\x00-\x7F]`) compile
// correctly. Every fixture language reaches full parity - including CSS and HTML,
// which diverged under the earlier UTF-16LE build.
...
void main() {
  ...
  final ffiHl = _build(const ShikiHighlighterNativeEngine());
  ...
  for (final c in golden) {
    ...
    test('native Oniguruma == pure-Dart · $lang/$theme', () {
      expect(
        _encode(ffiHl.codeToTokens(code, opts)),
