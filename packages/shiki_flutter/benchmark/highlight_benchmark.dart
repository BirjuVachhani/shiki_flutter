    ...
    (tester) async {
      ...
      final results = <String, dynamic>{
        'meta': {
          ...
          'cold_start_ms': double.parse(
            (coldStartMicros / 1000).toStringAsFixed(3),
          ),
          'note':
              'Headless flutter-test timings: relative comparison only. '
      ...
      for (final size in CorpusSize.values) {
        ...
        final tokenCount = tokens.fold<int>(
          0,
          (sum, line) => sum + line.length,
        );
        final maxTokensPerLine = tokens.fold<int>(
          0,
          (m, line) => line.length > m ? line.length : m,
        );
        final spanCount = tokenCount + (tokens.length - 1); // + newline spans
        final avgTokensPerLine = tokens.isEmpty
            ? 0.0
        ...
        const window = 50;
        final start = (tokens.length ~/ 2 - window ~/ 2).clamp(
          0,
          tokens.length,
        );
        ...
        final layoutWindow = measure(
          ...
          body: () {
            for (final s in windowSpans) {
              final tp = TextPainter(text: s, textDirection: TextDirection.ltr)
                ..layout(maxWidth: double.infinity);
              tp.dispose();
            }
          },
        ...
        results['sizes'][size.label] = {
          'structure': {
            ...
            'tokens': tokenCount,
            'spans': spanCount,
            'avg_tokens_per_line': double.parse(
              avgTokensPerLine.toStringAsFixed(2),
            ),
          ...
          'throughput': {
            ...
            'kb_per_sec': double.parse(
              (work.bytes / 1024 / (tokenize.medianMs / 1000)).toStringAsFixed(
                1,
              ),
            ),
          },
      ...
      results['meta']['rss_delta_mb'] = double.parse(
        rssDeltaMb.toStringAsFixed(1),
      );
...
void _printReport(Map<String, dynamic> r, int coldStartMicros, String path) {
  ...
  buf.writeln('\n${'=' * 78}');
  buf.writeln(
    'shiki_flutter headless benchmark  '
    '(lang=${r['meta']['lang']}, theme=${r['meta']['theme']})',
  );
  buf.writeln('=' * 78);
  buf.writeln(
    'Cold start (first ever highlight, incl. regex compile): '
    '${ms(coldStartMicros / 1000)} ms',
  );
  buf.writeln(
    'RSS delta over sweep: ${r['meta']['rss_delta_mb']} MB '
    '(coarse)',
  );
