...
void main() {
  test('isolate offloading: sync vs isolate-run vs warm-worker', () async {
    ...
    for (final size in sizes) {
      ...
      for (var i = 0; i < iters; i++) {
        final variant = '$code\n// r$i';
        runSamples.add(await _timeMicros(() async {
          await Isolate.run(() {
      ...
      for (var i = 0; i < iters; i++) {
        workerWarmSamples
            .add(await _timeMicros(() => worker.tokenize('$code\n// w$i', _opts)));
      }
    ...
    final snippets = [
      for (var i = 0; i < snippetCount; i++) '${corpusFor(CorpusSize.xs)}\n// n$i',
    ];

    final batchRunMs = await _timeMicros(() async {
    ...
    final batchWorker = await spawnTokenizeWorker(WorkerConfig(
      langs: [flattenBundledLanguage(dart)],
      themeJsons: [githubDark.json],
      warmups: const [WarmupSpec('dart', 'github-dark')],
    ));
    await batchWorker.ready;
    final batchWorkerMs = await _timeMicros(() async {
      for (final s in snippets) {
    ...
    final syncGap =
        await _maxHeartbeatGapMs(() async => warmHl.codeToTokens('$xl\n//j', _opts));

    final jankWorker = await spawnTokenizeWorker(WorkerConfig(
      langs: [flattenBundledLanguage(dart)],
      themeJsons: [githubDark.json],
      warmups: const [WarmupSpec('dart', 'github-dark')],
    ));
    ...
    final workerGap =
        await _maxHeartbeatGapMs(() => jankWorker.tokenize('$xl\n//j', _opts));
    ...
    print('\n==== tokenize latency (Dart / GitHub Dark), ms ====\n'
        '${table.render()}');
    // ignore: avoid_print
    print('==== $snippetCount small snippets, total ms ====\n'
    ...
    // ignore: avoid_print
    print('==== main-thread 16ms heartbeat, worst stall (xl) ====\n'
